...
void Audio::loop() {
    ...
    if(m_f_localfile) {       // Playing file fron SPIFFS or SD?
        //Serial.printf("LOOPB1\n");
        processLocalFile();
        //Serial.printf("LOOPB2\n");
        //reset();  
    }
    // - webstream - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
void Audio::processLocalFile() {
    //Serial.printf("LOOPB3\n");
    if(!(audiofile))  Serial.printf("audiofile: ");
    if(!(m_f_running))  Serial.printf("m_f_running: ");
    if(!(m_f_localfile))  Serial.printf("m_f_localfile: ");
    if(!(audiofile && m_f_running && m_f_localfile)) return;
    //Serial.printf("LOOPB4\n");
    int bytesDecoded = 0;
